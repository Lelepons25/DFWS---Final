...
#define TS_MINX 901 //100
#define TS_MAXX 137 //920
#define TS_MINY 79 //70
#define TS_MAXY 889 //900
#define STATUS_X 10
#define STATUS_Y 60
...
char currentPage;
int count = 0, calorie = 0;
int x = 0, y = 0, z = 0, totalIntake = 0, weight = 0, finalWeight = 0;
...
Adafruit_GFX_Button SaveButton[1], BackButton[1], NextButton[1], CancelButton[1], inputButton[1], tareButton[1], trackButton[1], ResetButton[1];
...
//PhilFCT  Philippine Food Composition Tables Online Database (PhilFCT®) 
//Food and Nutrition Research Institute
//Category Buttons
Adafruit_GFX_Button fruit[5];
char fruitlabels[5][24] = { "Apple","Avocado","Banana", "Durian", "Grape" };
int fruitcalories[5] = { 67, 100, 126, 163, 83};
uint16_t colors = ILI9341_esti2;

Adafruit_GFX_Button veg[5];
char veglabels[5][50] = { "Bamboo Shoot", "Banana Heart", "Bottle Gourd", "Cabbage", "Carrot" };
int vegcalories[5] = { 28, 39, 20, 31, 42 };

Adafruit_GFX_Button grains[5];
char grainslabels[5][50] = { "Bread", "Cashew", "Coconut Meat", "Mung Bean", "Oats"};
int grainscalories[5] = { 329, 603, 102, 348, 125};

Adafruit_GFX_Button protein[5];
char proteinlabels[5][50] = { "Chicken Breast", "Chicken Feet", "Chicken Leg", "Chicken Wing", "Beef Lean"};
int proteincalories[5] = {131, 187, 204, 194, 134 };

Adafruit_GFX_Button dairy[5];
char dairylabels[5][50] = { "Cheese", "Chic Spread", "Cream", "Buttermilk", "Cara Milk" };
int dairycalories[5] = { 297, 210, 241, 43, 124};
...
void loop() {
    ...
    if (currentPage == '0') {
        ...
        scale.update(); //if conversion is ready; read out 24 bit data and add to dataset
        int w = scale.getData(); //Get the value of the weight
        weight = computeWeight(w);
        DisplayWeight(weight); //Display the weight
    ...
    if (currentPage == '3') {
        ...
        weight = 0;
        DisplayWeight(weight); //Display zero since it is tare
        scale.start(1000); //restart the scale
        HomeScreen(); //Display the homescreen (currentpage = 0 )
    }
    //Tracker
    if (currentPage == '4') {
        ...
        if (ResetButton[1].justPressed()) {
            ResetButton[1].drawButton(true); // x = goal still remains
    ...
    if (currentPage == '5') {
        ...
        if (p.x >= 200 && p.x <= 275 && p.y >= 20 && p.y <= 40) { //Cancel Button
            clearInput();
        }
        if (p.x >= 10 && p.x <= 100 && p.y >= 295 && p.y <= 325) { //Back Button
            HomeScreen();
        }
        else {
            ...
            if (p.x >= 120 && p.x <= 340 && p.y >= 110 && p.y <= 140) {
                clearInput(); //Clears the text box every time the user clicks a fruit
                fruit[0].press(true);
                Serial.println("b = 1");
                count = 0;
                displayCalorie(0, categ); //To display the calorie (Passes 0 - firstelement of the array, categ - which category the element belongs
            ...
            if (p.x >= 120 && p.x <= 340 && p.y >= 262 && p.y <= 292) {
                ...
                if (p.x >= 200 && p.x <= 275 && p.y >= 60 && p.y <= 100) { // Save Button
                    ...
                    Serial.println("Save");
                    if (x == 0 || weight==0) { //User can't save if there's no input and weight
                        errorSave();
                    }
                    else {
                        ...
                        calorie = ceil(double((float)finalWeight / 100) * calorie); //Uses ceil to round up the computation
                        Serial.print("Calorie: ");
                        Serial.println(calorie);
                        totalIntake += calorie; 
                        Serial.println(totalIntake);
                        y = totalIntake;
                        z = x - y; //x = goal, y = intake, z = remaining calories
